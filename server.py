...
def _handle_feishu_event(event: dict):
    ...
    if msg_type == "image":
        ...
        result = run(
            ...
            f"═══════════════════════════════════════\n\n"
            f"【1. 家长调参指令】\n"
            f"如果消息含以下关键词（如'调整难度'、'增加题量'、'减少题量'、'更换教材'、'调整年级'、'查看学习报告'、'生成报告'、'难度提升'、'难度降低'、'增加写作'、'增加语法'等）：\n"
            f"- 先检查消息末尾是否有'家长密码：XXXX'或'密码：XXXX'格式\n"
            f"- 用 verify_password 验证密码\n"
            f"- 密码正确 → 读取 data/adjustments.json，按指令修改参数（如难度、题数、教材、年级等），保存后回复确认\n"
            f"- 密码错误或缺失 → 回复'请提供家长密码，格式：家长密码：XXXX'\n"
            f"- 如果是教材/年级变更 → 验证密码后用 web_search 联网搜该教材知识体系，自动生成 knowledge_map.json\n\n"
            f"【2. 答题/批改】\n"
            f"如果消息含'第X题答案是'、'答案是'、'选'等 → 按批改流程处理\n\n"
            f"【3. 普通对话】\n"
            f"以上都不是 → 友好回复\n\n"
            f"⚠️ 重要：处理完必须用 send_feishu(receive_id=\"{reply_target}\", msg_type=\"text\", content=\"...\") 发送结果，不要只返回不发送。",
            session,
        )
...
def scheduled_daily_push():
    ...
    try:
        ...
        result = run(
            ...
            "- 按 KET 四项能力权重出题：阅读35% + 写作15% + 听力25% + 口语25%\n"
            "- 每道题标注考核目标（语法点/词汇主题/阅读理解/听力/写作/口语）\n"
            "- 词汇控制在 KET 核心词表 ~1100 词内\n"
            "- 写作题需≥25词，给出范文模板；口语/听力题标注\"请口头回答\"\n"
            "- 包含：填空+选择+翻译/造句+完形填空+看图写作，题型混合\n"
            "- 每道题给出完整标准答案和纠错提示\n\n"
            "═══════════════════════════════════════\n"
