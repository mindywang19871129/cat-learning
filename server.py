...
def _handle_feishu_event(event: dict):
    ...
    if msg_type == "image":
        ...
        result = run(
            ...
            f"类型B·发新题 → 消息含 发题/新题/做题/再来一套 等 → 读adjustments/mastery/error_book/knowledge_map → call_llm出题 → write_file存today_questions.json（date必须用今天日期{datetime.now().strftime('%Y-%m-%d')}）→ send_feishu推送卡片\n"
